...
const makeTranslateDynamicSQL = (originalSQL, replaceSQL, replacePart, shouldVerify = true) => {
    if ( !originalSQL ) {
        return originalSQL;
    }
    if ( !replaceSQL ) {
...
const makeTranslateDynamicSQLValues = (originalSQL, transKeyValue = {} , shouldVerify = true) => {
    if ( !originalSQL ) {
        return originalSQL;
    }
    let result = originalSQL;
    Object.keys(transKeyValue).forEach( (v, idx, map) => {
        result = makeTranslateDynamicSQL(result, v, transKeyValue[v], shouldVerify );
    });
    return result;
}
