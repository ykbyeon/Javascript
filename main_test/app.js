...
app.use((req, res, next) => {
  console.log(
    `##    INTRO FILTER - 모든 패킷 통과 ::: [ CLIENT IP : ${req.ip} , REQUEST URL : ${req.url} ]`
  );
  next();
});
...
app.post(
  "/registUserInfo",
  asyncFn.asyncRouterWrapper(async (req, res, next) => {
...
app.use(async (req, res, next) => {
  ...
  if (!secureInfos || secureInfos.status === -1) {
    ...
    /*
        let user = undefined;
        if ( mainData.hasFreezeUser(userID) ) {
            user = mainData.getFreezeUserInfo(userID);
        } else {
            user = mainData.FreezingUserInfo(secureInfos.value.id, secureInfos.value.password);
            mainData.addFreezeUserInfo(user);
        }
...
app.use((req, res, next) => {
  res.status = 404;
  console.log(
    `##    FILE NOT FOUND - REQUEST INFO ::: [ CLIENT IP : ${req.ip} , REQUEST URL : ${req.url} ]`
  );
