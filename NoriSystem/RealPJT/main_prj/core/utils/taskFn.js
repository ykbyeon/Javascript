...
import { TASK_STATUS } from "#core/dto/taskInfo.js";
...
const execProcessForFileLog = async (cmd,args=[],options={},fileInfo={}, encoding) => {
    let prm = new Promise((resolve,reject) => {
        ...
        if ( !fileInfo.fileDir ) {
            if ( options.cwd ) {
                fileInfo.fileDir = options.cwd;
            } else {
                fileInfo.fileDir = path.resolve();
            }
        }
        ...
        ev.on ('close', (data) => {
            ...
            let ssv = (data === 0 ? "SUCCESS" : "FAIL");
            rev.status = 0;
            rev.data = result.length == 0 ? fileName : result.join("");
            rev.subStatus = data;
            rev.message = `NORMAL[${ssv}]`;
            resolve(rev);
        });
...
const execProcessByConfigs = async ( configs ) => {
    ...
    console.log ( ` MODE : ${mode} , OPTIONS : ${JSON.stringify(configs.options)}`);
    if ( (mode & 8) === 8 ) {
...
const executeFileExistsMonitoringFn = async ( analysisResult, monitorFunction={} ) => {
    ...
    try {
        ...
        return {status:TASK_STATUS.RUNNING, finished };
    }
}
...
const executeByConfigFileAnalysisFn = async ( executeResult, execConfigs ) => {
    if ( execConfigs && execConfigs.fileName ) {
        let fileDir = execConfigs.fileDir;
        if ( !fileDir ) {
            fileDir = process.cwd() ? process.cwd() : path.resolve();
        }
        return path.join(fileDir, execConfigs.fileName);
    }
    return executeResult;
}
...
export default {
    ...
    executeByPassAnalysisFn, executeByPassMonitoringFn, executeFileExistsMonitoringFn, executeByConfigFileAnalysisFn
};
