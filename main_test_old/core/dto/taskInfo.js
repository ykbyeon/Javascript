...
const MonitorTaskDTO = (taskID, execFnCfg={}, analysisFnCfg={}, monitorFnCfg={}, abortFnCfg={}, callbackFunction) => {
    ...
    const startTimeMills    = startDT.getTime();
    ...
    const executeMainTask = async () => {
        if ( isPrintLog ) {
            console.log ( `EXECUTE TASK : [ ID : ${taskID} ], [ DATE : ${(new Date()).toLocaleString()}]`);
        }
        if ( isExecuted ) {
            return;
        }
    ...
    const releaseResource = async () => {
        if ( callbackFinishedFunction ) {
            await callbackFinishedFunction(_taskID, getStatus(), analysisResult);
        }
        if ( timerID ) {
    ...
    return Object.freeze({
        ...
        getTaskID , getStartDateStr, setCallbackFunction 
    });
};
...
const MonitorTaskVO = (tGroupID, taskDtoArray = [], execDelayMillis = 100, callbackFunction ) => {
    ...
    const setTaskFinished = ( taskID, taskStatus )  => {
        ...
        if ( finishedAll ) {
            if ( callbackFinishedFunction ) {
                callbackFinishedFunction(taskGroupID, taskVOStatus);
            }
        }
    };
    ...
    const setTaskGroupFinished = ( taskGgroupID, groupStatus ) => {
        ...
        if ( groupStatus === TASK_STATUS.DONE ) {
            ...
            if ( callbackFinishedFunction ) {
                ...
                if ( callbackFinishedFunction ) {
                    setTimeout ( async () => {
                        await callbackFinishedFunction(jobID, jobStatus);
...
const makeCallbackSampleFunction = async ( taskID, taskStatus, taskKeyValue ) => {
