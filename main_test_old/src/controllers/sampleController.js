...
const sampleCtrlTest = async (jsonParams) => {
    let simpleData ;
    let simpleInData;
    let serviceStatus = -1;
    try { 
        simpleData = await sampleService.getTestValue(jsonParams.simpleData);
        simpleInData = await sampleService.getTestDynamicInValues(jsonParams.simpleInData);
        serviceStatus = 0;
    } catch ( e ) {
        serviceStatus = -1;
        console.log ( e );
    ...
    const data = {
        'simpleResult' : simpleData, 
        'simpleInResult' : simpleInData
    }
...
export default {
    sampleCtrlTest, closeResource, sampleCtrlDTOTest, getSampleDataListAll, modifySampleMasterDetailValues
};